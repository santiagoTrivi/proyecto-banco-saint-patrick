...
export class RegisterClient implements CreateEntity<ClientEntity> {
    ...
    run = async({firstName, lastName, username, password}: ClientEntity): Promise<IAuthentication> => {

        const IS_USERNAME_REGISTERED  = await this.clientService.findOne({username});
        ...
        const client = new ClientEntity(undefined, firstName, lastName, username, hashedPassword);
